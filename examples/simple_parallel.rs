...
impl Task for TaskA {
    async fn run(&self, context: Context) -> Result<(), GraphError> {
        ...
        ctx.set("pipeline_started", true);
        ctx.set("start_time", current_timestamp());
...
impl Task for TaskB {
    async fn run(&self, context: Context) -> Result<(), GraphError> {
        ...
        
        {
            ...
            ctx.set("task_b_completion_time", completion_time);
        }
...
impl Task for TaskC {
    async fn run(&self, context: Context) -> Result<(), GraphError> {
        ...
        
        {
            ...
            ctx.set("task_c_completion_time", completion_time);
        }
...
impl Task for TaskD {
    async fn run(&self, context: Context) -> Result<(), GraphError> {
        ...
        let (task_b_time, task_c_time) = {
            let ctx = context.read().await;
            let b_time = ctx.get::<u64>("task_b_completion_time").copied().unwrap_or(0);
            let c_time = ctx.get::<u64>("task_c_completion_time").copied().unwrap_or(0);
            (b_time, c_time)
        };
        ...
        {
            let mut ctx = context.write().await;
            ctx.set("random_number", random_number);
            ctx.set("is_prime", is_prime_number);
...
impl Task for TaskE {
    async fn run(&self, context: Context) -> Result<(), GraphError> {
        let ctx = context.read().await;
        let random_number = ctx.get::<u32>("random_number").copied().unwrap_or(0);
        
        println!("🎯 Task E: Final task executing!");
        println!("🎉 Task E: Successfully reached because {} is prime!", random_number);
        
        drop(ctx);
        
        {
            let mut ctx = context.write().await;
            ctx.set("task_e_completed", true);
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let prime_condition: Condition = Arc::new(|context: &ExtendedContext| {
        ...
        println!("🔍 Condition Check: Is {} prime? {}", random_number, is_prime);
        is_prime
    });
    ...
    graph
        ...
        .add_cond_edge(task_d.clone(), task_e.clone(), prime_condition, Option::<TaskE>::None)?; // D -> E (if prime)
    ...
    // Check final results
    {
        ...
        let task_e_completed = ctx_guard.get::<bool>("task_e_completed").copied().unwrap_or(false);
        let random_number = ctx_guard.get::<u32>("random_number").copied().unwrap_or(0);
        let is_prime = ctx_guard.get::<bool>("is_prime").copied().unwrap_or(false);
